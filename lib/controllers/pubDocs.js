...
exports.update = function (req, res) {
  ...
  if (req.user) {
    ...
    if (type == 'repost') {
      if (data) {
        if (!pubDoc.meta._reposts) {
          pubDoc.meta._reposts = [];
        }
        pubDoc.meta._reposts.addToSet(req.user._id);
        users.addRepost(req.user._id, pubDoc._id);         
        pubDoc.reposts++;
      } else {
        pubDoc.meta._reposts.pull(req.user._id);
        users.removeRepost(req.user._id, pubDoc._id);
        pubDoc.reposts--;
      } 
    } 
