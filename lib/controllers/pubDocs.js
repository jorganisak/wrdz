  ...
  if (req.user) {
    ...
    if (type === 'view') {
      if (data) {
        ...
        User.findByIdAndUpdate(req.user._id, { $push: { 'meta._views': pubDoc._id }},
          function (err) {
            if (err) {
    ...
    if (type == 'up_vote') {
         if (data) {
          ...
          User.findByIdAndUpdate(req.user._id, { $pull: { 'meta._up_votes': pubDoc._id }}, 
            function (err) {
              if (err) { 
                return console.log(err);
              }
    ...
    if (type == 'heart') {
        if (data) {
          ...
          User.findByIdAndUpdate(req.user._id, { $pull: { 'meta._hearts': pubDoc._id }}, 
            function (err) {
...
exports.list = function  (req, res) {
  ...
  PubDoc
    ...
    .sort('score')
    .limit(20)
    .populate('doc', 'title has_title', null, null)
